...
public class CatController : MonoBehaviour
{
    ...
    private void SetChaseMovement()
    {
        ...
        {
            //Catched th chick
            _catStateController.ChangeState(CatState.Atacking);
            _isChasing = false;
        }
    ...
    private void SetRandomDestination()
    {
        ...
        if(!destinationSet)
        {
            Debug.LogWarning("Failed to find a valid destination after multiple attempts.");
            _isWaiting = true;
            _timer = _waitTime * 2;
        }
    ...
    {
        NavMeshHit hit;
        if (NavMesh.Raycast(transform.position, position, out hit, NavMesh.AllAreas))
        {
            return true;
        }
        return false;
