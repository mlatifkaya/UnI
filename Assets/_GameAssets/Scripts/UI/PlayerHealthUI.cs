...
public class PlayerHealthUI : MonoBehaviour
{
    ...
    private RectTransform[] _playerHealthRectTransform;
    private void Awake()
    {
        _playerHealthRectTransform = new RectTransform[_playerHealthImages.Length];
        for (int i = 0; i < _playerHealthImages.Length; i++)
        {
            _playerHealthRectTransform[i] = _playerHealthImages[i].GetComponent<RectTransform>();
        }
    }
    ...
    public void AnimateDamage()
    {
        for(int i=0; i < _playerHealthImages.Length; i++)
        {
            ...
            {
                AnimateDamageSprite(_playerHealthImages[i], _playerHealthRectTransform[i]);
                break;
            }
        }
    }
